...
public interface AlunoRepository extends JpaRepository<Aluno, Long> {
	@Query(value = "select aluno.id,aluno.data_de_matricula,aluno.data_de_nascimento,aluno.name,aluno.second_name "
			+ "from aluno LEFT JOIN  mensalidade " + "ON aluno.id = mensalidade.aluno_id "
			+ "WHERE NOT(EXTRACT(MONTH FROM(mensalidade.data_pagamento))=:mes) " + "union "
			...
			, countQuery = "select count(*) as total_linhas from (select aluno.id,aluno.data_de_matricula,aluno.data_de_nascimento,aluno.name,aluno.second_name "
					+ "from aluno LEFT JOIN  mensalidade " + "ON aluno.id = mensalidade.aluno_id "
					+ "WHERE NOT(EXTRACT(MONTH FROM(mensalidade.data_pagamento))=:mes) " + "union "
					+ "select a.id,a.data_de_matricula,a.data_de_nascimento,a.name,a.second_name from aluno a "
					+ "where not exists (select m from mensalidade m where m.aluno_id=a.id))" + " as sub_consulta",
...
//              "AND NOT EXISTS (SELECT m2 FROM Mensalidade m2 WHERE m2.aluno = a)")
	Page<Aluno> findAllAlunosDevedores(int mes, PageRequest pr);
